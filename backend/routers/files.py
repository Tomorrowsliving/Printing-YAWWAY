...
async def list_files(printer_slug: str, file_type: str):
    ...
    target_path = os.path.abspath(os.path.join(STORAGE_ROOT, printer_slug, file_type))
    abs_root = os.path.abspath(STORAGE_ROOT)

    # Security: Prevent path traversal
    if os.path.commonpath([target_path, abs_root]) != abs_root:
...
async def read_file(path: str):
    ...
    # Security: Ensure path is within STORAGE_ROOT and prevent path traversal
    if os.path.commonpath([target_path, abs_root]) != abs_root:
        raise HTTPException(status_code=403, detail="Access denied")
...
async def upload_file(printer_slug: str, file_type: str, file: UploadFile = File(...)):
    target_dir = os.path.abspath(os.path.join(STORAGE_ROOT, printer_slug, file_type))
    abs_root = os.path.abspath(STORAGE_ROOT)

    # Security: Prevent path traversal
    if os.path.commonpath([target_dir, abs_root]) != abs_root:
        raise HTTPException(status_code=403, detail="Access denied")
